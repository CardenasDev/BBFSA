  ...
  template: `
    <div class="page-heading">
      <div>
        ...
        <p class="muted">
          Seguimiento de ingreso, contratos, seguridad social, examenes y documentos.
        </p>
      </div>
      @if (auth.hasPermission('CONTRATACION_ALERTAS_VER')) {
        <a class="btn secondary" routerLink="/admin/contracting/alerts">Alertas</a>
      }
    </div>
        ...
        <table>
          ...
          <tbody>
            @for (employee of employees(); track employee.id_empleado) {
              <tr>
                <td>
                  <strong>{{ employee.numero_documento }}</strong
                  ><small class="muted">{{ employee.estado_empleado || 'Sin estado' }}</small>
                </td>
                ...
                <td>
                  <span
                    class="badge"
                    [class.success]="employee.estado_contrato === 'ACTIVO'"
                    [class.danger]="employee.estado_contrato === 'VENCIDO'"
                    >{{ employee.estado_contrato || 'Sin contrato' }}</span
                  >
                  <small class="muted">{{ employee.tipo_contrato || '' }}</small>
                  <small class="muted"
                    >{{ employee.numero_contrato || 'Sin numero' }} -
                    {{ employee.tipo_cargo_contrato || 'Sin tipo cargo' }}</small
                  >
                  <small class="muted"
                    >{{ employee.periodo_pago || 'Sin periodo' }} -
                    {{ employee.lugar_labores || 'Sin lugar' }}</small
                  >
                </td>
                <td>
                  <span
                    class="badge"
                    [class.success]="!truthy(employee.ficha_pendiente)"
                    [class.danger]="truthy(employee.ficha_pendiente)"
                    >{{
                      employee.estado_ficha ||
                        (truthy(employee.ficha_pendiente) ? 'PENDIENTE' : 'Sin dato')
                    }}</span
                  >
                </td>
                ...
                <td>
                  <div class="chip-list">
                    @if (truthy(employee.ficha_pendiente)) {
                      <span class="chip">Ficha pendiente</span>
                    }
                    @if (truthy(employee.contrato_proximo_vencer)) {
                      <span class="chip">Contrato proximo</span>
                    }
                    @if (truthy(employee.contrato_vencido)) {
                      <span class="chip">Contrato vencido</span>
                    }
                  </div>
                </td>
                <td>
                  <div class="row-actions">
                    @if (auth.hasPermission('CONTRATACION_VER')) {
                      <a
                        class="btn small primary"
                        [routerLink]="[
                          '/admin/contracting/employees',
              ...
              <tr>
                <td colspan="9" class="empty">
                  {{
                    loading()
                      ? 'Cargando informacion de contratacion...'
