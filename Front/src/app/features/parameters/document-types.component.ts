...
@Component({
  ...
  imports: [FormsModule],
  template: `
    <div class="page-heading"><div><p class="eyebrow">Parámetros</p><h1>Tipos de documento</h1></div>
    ...
    <section class="panel">
      ...
      <div class="table-wrap"><table><thead><tr><th>Nombre</th><th>Estado</th><th>Acciones</th></tr></thead>
        <tbody>
          @for (d of items(); track d.id_tipo_documento) {
            <tr><td><strong>{{ d.nombre }}</strong></td><td><span class="badge" [class.success]="d.activo">{{ d.activo ? 'ACTIVO' : 'INACTIVO' }}</span></td>
...
export class ParametersDocumentTypesComponent implements OnInit {
  ...
  load(){ this.error.set(''); this.svc.listDocumentTypes().subscribe({ next:(r)=>this.items.set(r), error:(e)=>this.error.set(apiErrorMessage(e,'No fue posible cargar.')) }); }
  ...
  openEdit(d:any){ this.formError.set(''); this.form={nombre:d.nombre,descripcion:d.descripcion??''}; this.editingId=d.id_tipo_documento; this.editMode.set(true); this.createOpen.set(true); }
  closeCreate(){ this.createOpen.set(false); }
  save(){ const nombre=this.form.nombre?.trim(); if(!nombre){ this.formError.set('El nombre es obligatorio'); return;} this.loading.set(true); const op = this.editingId? this.svc.updateDocumentType(this.editingId,{nombre:this.form.nombre, descripcion:this.form.descripcion}) : this.svc.createDocumentType({nombre:this.form.nombre, descripcion:this.form.descripcion}); op.pipe(finalize(()=>this.loading.set(false))).subscribe({ next:()=>{ this.closeCreate(); this.load(); }, error:(e)=>this.formError.set(apiErrorMessage(e,'No fue posible guardar.')) }); }
  toggleState(d:any){ if(d.activo && !confirm(`¿Desea inactivar ${d.nombre}?`)) return; this.loading.set(true); this.svc.updateDocumentType(d.id_tipo_documento,{ nombre:d.nombre, descripcion:d.descripcion??'', activo: !d.activo }).pipe(finalize(()=>this.loading.set(false))).subscribe({ next:()=>this.load(), error:(e)=>this.error.set(apiErrorMessage(e,'No fue posible cambiar el estado.')) }); }
}
