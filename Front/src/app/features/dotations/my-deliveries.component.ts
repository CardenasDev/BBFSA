  ...
  template: `
    <div class="page-heading">
      <div>
        ...
        <h1>Mis dotaciones</h1>
        <p class="muted">Consulta las dotaciones que te han entregado y confirma recibido.</p>
      </div>
    </div>
    ...
    <section class="panel">
      ...
      @for (delivery of deliveries(); track delivery.id_dotacion_entrega) {
        <article class="drawer-section">
          <div class="section-title">
            <div>
              ...
              <p class="muted">
                Fecha entrega: {{ delivery.fecha_entrega }} - Registrado por:
                {{
                  delivery.registrado_por ||
                    (delivery.id_registrado_por ? 'ID ' + delivery.id_registrado_por : 'Sin dato')
                }}
              </p>
            </div>
            <span
              class="badge"
              [class.success]="delivery.estado === 'ENTREGADA'"
              [class.danger]="delivery.estado === 'ANULADA'"
              >{{ statusLabel(delivery.estado) }}</span
            >
          </div>
          ...
          <dl>
            ...
            <dd>
              {{ delivery.tipo_entrega === 'EXTRAORDINARIA' ? 'Extraordinaria' : 'Ordinaria' }}
            </dd>
            <dt>Combinacion</dt>
            <dd>{{ combinationLabel(delivery) }}</dd>
            <dt>Observaciones entrega</dt>
            ...
            <dd>{{ delivery.observacion_confirmacion || 'Sin observacion' }}</dd>
          </dl>

          <div class="row-actions">
            @if (evidenceUrl(delivery); as url) {
              <a class="btn small secondary" [href]="url" target="_blank" rel="noopener noreferrer"
                >Ver evidencia</a
              >
            }
            <button
              ...
              type="button"
              (click)="toggleDetails(delivery)"
              [disabled]="detailsLoadingId() === delivery.id_dotacion_entrega"
            >
              {{
                isExpanded(delivery.id_dotacion_entrega)
                  ? 'Ocultar detalle'
                  : detailsLoadingId() === delivery.id_dotacion_entrega
                    ? 'Cargando...'
                    : 'Ver detalle'
              }}
            </button>
            @if (canConfirm(delivery)) {
              <button class="btn small primary" type="button" (click)="openConfirm(delivery)">
                Confirmar recibido
              </button>
            }
          </div>
          ...
          @if (isExpanded(delivery.id_dotacion_entrega)) {
            <div class="table-wrap">
              <table>
                ...
                <tbody>
                  @for (
                    detail of deliveryDetails(delivery.id_dotacion_entrega);
                    track detail.id_dotacion_entrega_detalle
                  ) {
                    <tr>
                      <td>
                    ...
                    <tr>
                      <td colspan="5" class="empty">
                        {{
                          detailsLoadingId() === delivery.id_dotacion_entrega
                            ? 'Cargando detalle...'
                            : 'No hay items registrados para esta entrega.'
                        }}
                      </td>
                    </tr>
                  }
        ...
        <div class="empty tall">
          {{ loading() ? 'Cargando dotaciones...' : 'No tienes entregas de dotacion registradas.' }}
        </div>
      }
    </section>
    ...
    @if (selectedDelivery(); as delivery) {
      ...
      <aside class="role-drawer" aria-label="Confirmar recibido" aria-modal="true">
        ...
        @if (confirmError()) {
          <div class="alert error" role="alert">{{ confirmError() }}</div>
        }

        <label
          >Observacion de confirmacion
          <textarea
            rows="5"
            maxlength="500"
            name="observacion_confirmacion"
            [(ngModel)]="confirmationObservation"
            [disabled]="confirming()"
            placeholder="Recibido completo y en buen estado."
          ></textarea>
        </label>
...
export class MyDotationDeliveriesComponent implements OnInit {
  ...
  canConfirm(delivery: MyDotationDelivery): boolean {
    return (
      ...
      this.auth.hasPermission('DOTACIONES_MIS_ENTREGAS_CONFIRMAR')
    );
  }
  ...
  load(): void {
    ...
    this.service
      ...
      .subscribe({
        ...
        error: (error) =>
          this.error.set(apiErrorMessage(error, 'No fue posible cargar tus entregas de dotacion.')),
      });
  }
  ...
  toggleDetails(delivery: MyDotationDelivery): void {
    ...
    this.service
      ...
      .pipe(finalize(() => this.detailsLoadingId.set(null)))
      .subscribe({
        next: (details) =>
          this.detailsByDelivery.update((current) => ({ ...current, [deliveryId]: details })),
        error: (error) =>
          this.detailErrors.update((current) => ({
            ...current,
            [deliveryId]: apiErrorMessage(
              error,
              'No fue posible cargar el detalle de esta entrega.',
            ),
          })),
      });
  }
  ...
  confirmReceived(): void {
    ...
    this.confirmError.set('');
    this.service
      .confirmDeliveryReceived(delivery.id_dotacion_entrega, {
      ...
      .subscribe({
        next: (confirmed) => {
          ...
          this.deliveries.update((deliveries) =>
            deliveries.map((item) =>
              item.id_dotacion_entrega === delivery.id_dotacion_entrega
                ? {
                    ...item,
