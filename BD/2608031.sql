...
BEGIN
    IF UPPER(TRIM(P_TIPO_DEVOLUCION))
       NOT IN ('DOTACION','HERRAMIENTA') THEN
        SIGNAL SQLSTATE '45000'
    ...
    IF UPPER(TRIM(P_TIPO_DEVOLUCION)) = 'DOTACION' THEN
        SELECT
            'DOTACION' AS TIPO_DEVOLUCION,
            DE.ID_DOTACION_ENTREGA AS ID_ENTREGA,
            DD.ID_DOTACION_ENTREGA_DETALLE AS ID_DETALLE,
            DE.ID_EMPLEADO,
            DE.FECHA_ENTREGA,
            TD.NOMBRE AS ELEMENTO,
            T.TALLA,
            DD.CANTIDAD AS CANTIDAD_ENTREGADA,
            COALESCE(DV.CANTIDAD_RESERVADA, 0) AS CANTIDAD_DEVUELTA,
            DD.CANTIDAD - COALESCE(DV.CANTIDAD_RESERVADA, 0)
        ...
        LEFT JOIN (
            SELECT
                ...
                SUM(DDET.CANTIDAD_DEVUELTA) AS CANTIDAD_RESERVADA
            FROM bbf_devolucion_detalle DDET
            INNER JOIN bbf_devoluciones DEV
                ON DEV.ID_DEVOLUCION = DDET.ID_DEVOLUCION
               AND DEV.ESTADO IN ('REGISTRADA','CONFIRMADA')
            GROUP BY DDET.ID_DOTACION_ENTREGA_DETALLE
        ) DV
        ...
        WHERE DE.ID_EMPLEADO = P_ID_EMPLEADO
          ...
          AND DD.CANTIDAD > COALESCE(DV.CANTIDAD_RESERVADA, 0)
        ORDER BY DE.FECHA_ENTREGA DESC,
                 DE.ID_DOTACION_ENTREGA DESC,
                 TD.NOMBRE ASC;
    ELSE
        SELECT
            'HERRAMIENTA' AS TIPO_DEVOLUCION,
            HE.id_entrega AS ID_ENTREGA,
            HD.id_detalle AS ID_DETALLE,
            HE.id_empleado AS ID_EMPLEADO,
            HE.fecha_entrega AS FECHA_ENTREGA,
            H.nombre AS ELEMENTO,
            NULL AS TALLA,
            HD.cantidad AS CANTIDAD_ENTREGADA,
            COALESCE(DV.CANTIDAD_RESERVADA, 0) AS CANTIDAD_DEVUELTA,
            HD.cantidad - COALESCE(DV.CANTIDAD_RESERVADA, 0)
                AS CANTIDAD_DISPONIBLE
        ...
        LEFT JOIN (
            ...
            INNER JOIN bbf_devoluciones DEV
                ON DEV.ID_DEVOLUCION = DDET.ID_DEVOLUCION
               AND DEV.ESTADO IN ('REGISTRADA','CONFIRMADA')
            GROUP BY DDET.ID_HERRAMIENTA_ENTREGA_DETALLE
        ) DV
            ON DV.ID_HERRAMIENTA_ENTREGA_DETALLE = HD.id_detalle
        WHERE HE.id_empleado = P_ID_EMPLEADO
          AND HE.estado = 'confirmada'
          AND HD.cantidad > COALESCE(DV.CANTIDAD_RESERVADA, 0)
...
BEGIN
    ...
    INNER JOIN bbf_empleados E
        ON E.ID_EMPLEADO = DEV.ID_EMPLEADO
    WHERE DEV.ID_DEVOLUCION = P_ID_DEVOLUCION;
    ...
    SELECT
        ...
        DET.ID_DOTACION_ENTREGA_DETALLE,
        DET.ID_HERRAMIENTA_ENTREGA_DETALLE,
        COALESCE(TD.NOMBRE, H.nombre) AS ELEMENTO,
        T.TALLA,
    ...
    LEFT JOIN bbf_herramientas_entrega_detalle HD
        ON HD.id_detalle = DET.ID_HERRAMIENTA_ENTREGA_DETALLE
    LEFT JOIN bbf_herramientas H
        ON H.id_herramienta = HD.id_herramienta
    WHERE DET.ID_DEVOLUCION = P_ID_DEVOLUCION
    ...
    FROM bbf_devolucion_evidencias
    WHERE ID_DEVOLUCION = P_ID_DEVOLUCION
    ORDER BY ID_DEVOLUCION_EVIDENCIA ASC;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
BEGIN
    ...
    /*
     * Reporte informativo para cotización de dotación.
     *
     * Devuelve:
     * - Talla actual registrada por empleado y prenda.
     * - Última entrega confirmada de esa misma prenda.
     * - Talla y cantidad entregadas en esa ocasión.
     *
     * Solo considera:
     * - Empleados activos y no eliminados.
     * - Entregas confirmadas con estado ENTREGADA.
     * - Cabeceras y detalles no eliminados.
     *
     * Los parámetros aceptan NULL o 0 para no aplicar el filtro.
     */
    ...
    SELECT
        ...
        EDT.ID_TALLA_DOTACION AS ID_TALLA_ACTUAL,
        TA.TALLA AS TALLA_ACTUAL,

        UE.ID_DOTACION_ENTREGA AS ID_ULTIMA_ENTREGA,
        UE.FECHA_ULTIMA_ENTREGA,
        UE.TIPO_ULTIMA_ENTREGA,
        UE.FECHA_CONFIRMACION,

        UE.ID_TALLA_ULTIMA_ENTREGA,
        UE.TALLA_ULTIMA_ENTREGA,
        UE.CANTIDAD_ULTIMA_ENTREGA,
    ...
    LEFT JOIN bbf_tallas_dotacion TA
        ON TA.ID_TALLA_DOTACION = EDT.ID_TALLA_DOTACION
       AND TA.ID_TIPO_DOTACION = EDT.ID_TIPO_DOTACION
    ...
    LEFT JOIN (
        SELECT
            ...
            ID_EMPLEADO,
            ID_TIPO_DOTACION,
            FECHA_ENTREGA AS FECHA_ULTIMA_ENTREGA,
            TIPO_ENTREGA AS TIPO_ULTIMA_ENTREGA,
            FECHA_CONFIRMACION,
            ID_TALLA_DOTACION AS ID_TALLA_ULTIMA_ENTREGA,
            TALLA_ULTIMA_ENTREGA,
            CANTIDAD AS CANTIDAD_ULTIMA_ENTREGA,
            OBSERVACIONES_ULTIMA_ENTREGA
        FROM ULTIMAS_ENTREGAS
        WHERE NUMERO_FILA = 1
    ) UE
        ON UE.ID_EMPLEADO = EDT.ID_EMPLEADO
       AND UE.ID_TIPO_DOTACION = EDT.ID_TIPO_DOTACION

    WHERE (P_ID_AREA IS NULL OR P_ID_AREA = 0
           OR E.ID_AREA = P_ID_AREA)

      AND (P_ID_CARGO IS NULL OR P_ID_CARGO = 0
           OR E.ID_CARGO = P_ID_CARGO)

      AND (P_ID_EMPLEADO IS NULL OR P_ID_EMPLEADO = 0
...
BEGIN
    SELECT
        ...
        E.ID_EMPLEADO,
        E.NUMERO_DOCUMENTO,
        CONCAT_WS(' ', E.NOMBRES, E.APELLIDOS)
            AS NOMBRE_COMPLETO,
        ...
        DE.OBSERVACIONES AS OBSERVACIONES_SOLICITUD,
...
BEGIN
    ...
    SELECT ID_EMPLEADO, ESTADO, ELIMINADO
    INTO V_ID_EMPLEADO_ENTREGA, V_ESTADO, V_ELIMINADO
    FROM bbf_dotacion_entregas
    ...
    IF V_ID_EMPLEADO_ENTREGA IS NULL THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT =
            'La entrega de dotación no existe.';
    END IF;
    ...
    IF V_ESTADO = 'POR_COMPRAR' THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT =
            'La dotación todavía se encuentra por comprar y no puede confirmarse.';
    END IF;
    ...
    IF V_ESTADO <> 'REGISTRADA' THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT =
            'La entrega no se encuentra disponible para confirmación.';
    END IF;
...
BEGIN
    ...
    SET V_ESTADO_INICIAL =
        UPPER(TRIM(IFNULL(P_ESTADO_INICIAL, 'REGISTRADA')));
        ...
        CASE
            WHEN V_TIENE_URL = 1
              ...
              OR (
                ...
                AND TRIM(P_EVIDENCIA_NOMBRE_ARCHIVO) <> ''
              )
            THEN 1
            ELSE 0
        END;
    ...
    -- --------------------------------------------------------
    -- VALIDAR FECHA
    -- Para POR_COMPRAR se interpreta como fecha requerida.
    -- --------------------------------------------------------
    ...
    IF V_ESTADO_INICIAL NOT IN ('POR_COMPRAR', 'REGISTRADA') THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT =
    ...
    IF V_TIPO_ENTREGA NOT IN ('ORDINARIA', 'EXTRAORDINARIA') THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT =
            'El tipo de entrega debe ser ORDINARIA o EXTRAORDINARIA.';
    END IF;
    ...
    IF V_TIPO_ENTREGA = 'ORDINARIA' THEN
        IF P_ID_DOTACION_COMBINACION IS NULL
           OR P_ID_DOTACION_COMBINACION <= 0 THEN
            SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT =
        ...
        IF V_EXISTE_COMBINACION = 0 THEN
            SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT =
                'La combinación seleccionada no existe o está inactiva.';
        END IF;
    ...
    IF V_ESTADO_INICIAL = 'REGISTRADA' THEN
        ...
        IF V_TIENE_URL = 0 AND V_TIENE_RUTA = 0 THEN
            SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT =
                'Debe registrar una URL externa o cargar un archivo como evidencia.';
        END IF;

        IF V_TIENE_URL = 1 AND V_TIENE_RUTA = 1 THEN
            SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT =
                'La evidencia debe ser una URL externa o un archivo físico, no ambos.';
        END IF;
    ...
    VALUES (
        ...
        CASE
            ...
            THEN NULLIF(TRIM(P_EVIDENCIA_NOMBRE_ARCHIVO), '')
            ELSE NULL
        END,

        CASE
            WHEN V_ESTADO_INICIAL = 'REGISTRADA'
            THEN NULLIF(TRIM(P_EVIDENCIA_NOMBRE_ORIGINAL), '')
            ELSE NULL
        END,

        CASE
            WHEN V_ESTADO_INICIAL = 'REGISTRADA'
            THEN NULLIF(TRIM(P_EVIDENCIA_URL), '')
            ELSE NULL
        END,

        CASE
            WHEN V_ESTADO_INICIAL = 'REGISTRADA'
            THEN NULLIF(TRIM(P_EVIDENCIA_RUTA), '')
            ELSE NULL
        END,
...
BEGIN
    ...
    IF V_ESTADO IS NULL THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT =
            'La solicitud de dotación no existe.';
    END IF;
    ...
    SET
        ...
        EVIDENCIA_NOMBRE_ARCHIVO =
            NULLIF(TRIM(P_EVIDENCIA_NOMBRE_ARCHIVO), ''),

        EVIDENCIA_NOMBRE_ORIGINAL =
            NULLIF(TRIM(P_EVIDENCIA_NOMBRE_ORIGINAL), ''),
...
-- Dump completed on 2026-08-03 11:25:00
