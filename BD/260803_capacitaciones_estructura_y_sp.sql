...
DROP PROCEDURE IF EXISTS `SP_BBF_CAPACITACION_EVIDENCIA_CREAR`$$
CREATE PROCEDURE `SP_BBF_CAPACITACION_EVIDENCIA_CREAR`(
    IN P_ID_SESION INT, IN P_ID_PARTICIPANTE INT, IN P_ID_COMPROMISO INT,
    IN P_TIPO_EVIDENCIA VARCHAR(30), IN P_NOMBRE_ARCHIVO VARCHAR(255), IN P_NOMBRE_ORIGINAL VARCHAR(255),
    IN P_ARCHIVO_RUTA VARCHAR(500), IN P_ARCHIVO_URL VARCHAR(500), IN P_MIME_TYPE VARCHAR(100), IN P_PESO_BYTES BIGINT,
    IN P_ID_USUARIO INT
)
BEGIN
    DECLARE V_TIPO VARCHAR(30);
    SET V_TIPO = UPPER(TRIM(IFNULL(P_TIPO_EVIDENCIA,'OTRA')));
    IF V_TIPO NOT IN('ASISTENCIA','CONFIRMACION','EVALUACION','FIRMA','COMPROMISO','OTRA') THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='Tipo de evidencia no valido.';
    END IF;
    IF P_ID_SESION IS NULL OR NOT EXISTS(SELECT 1 FROM bbf_capacitacion_sesiones WHERE ID_CAPACITACION_SESION=P_ID_SESION) THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='La sesion no existe.';
    END IF;
    IF NULLIF(TRIM(P_ARCHIVO_RUTA),'') IS NULL AND NULLIF(TRIM(P_ARCHIVO_URL),'') IS NULL THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='La evidencia requiere una ruta o una URL valida.';
    END IF;
    INSERT INTO bbf_capacitacion_evidencias(ID_CAPACITACION_SESION,ID_CAPACITACION_PARTICIPANTE,ID_CAPACITACION_COMPROMISO,
    TIPO_EVIDENCIA,NOMBRE_ARCHIVO,NOMBRE_ORIGINAL,ARCHIVO_RUTA,ARCHIVO_URL,MIME_TYPE,PESO_BYTES,ID_CARGADO_POR)
    VALUES(P_ID_SESION,NULLIF(P_ID_PARTICIPANTE,0),NULLIF(P_ID_COMPROMISO,0),V_TIPO,
    NULLIF(TRIM(P_NOMBRE_ARCHIVO),''),NULLIF(TRIM(P_NOMBRE_ORIGINAL),''),NULLIF(TRIM(P_ARCHIVO_RUTA),''),NULLIF(TRIM(P_ARCHIVO_URL),''),
    NULLIF(TRIM(P_MIME_TYPE),''),P_PESO_BYTES,P_ID_USUARIO);
    SELECT * FROM bbf_capacitacion_evidencias WHERE ID_CAPACITACION_EVIDENCIA=LAST_INSERT_ID();
END$$

DROP PROCEDURE IF EXISTS `SP_BBF_CAPACITACION_EVIDENCIAS_LISTAR`$$
CREATE PROCEDURE `SP_BBF_CAPACITACION_EVIDENCIAS_LISTAR`(IN P_ID_SESION INT)
BEGIN
    SELECT E.*
    FROM bbf_capacitacion_evidencias E
    WHERE E.ID_CAPACITACION_SESION=P_ID_SESION
    ORDER BY E.CREATED_AT DESC, E.ID_CAPACITACION_EVIDENCIA DESC;
END$$

DROP PROCEDURE IF EXISTS `SP_BBF_CAPACITACION_EVIDENCIA_OBTENER`$$
CREATE PROCEDURE `SP_BBF_CAPACITACION_EVIDENCIA_OBTENER`(IN P_ID_SESION INT, IN P_ID_EVIDENCIA BIGINT)
BEGIN
    SELECT E.*
    FROM bbf_capacitacion_evidencias E
    WHERE E.ID_CAPACITACION_SESION=P_ID_SESION AND E.ID_CAPACITACION_EVIDENCIA=P_ID_EVIDENCIA;
END$$

DROP PROCEDURE IF EXISTS `SP_BBF_CAPACITACION_EVIDENCIA_ELIMINAR`$$
CREATE PROCEDURE `SP_BBF_CAPACITACION_EVIDENCIA_ELIMINAR`(IN P_ID_SESION INT, IN P_ID_EVIDENCIA BIGINT)
BEGIN
    SELECT * FROM bbf_capacitacion_evidencias
    WHERE ID_CAPACITACION_SESION=P_ID_SESION AND ID_CAPACITACION_EVIDENCIA=P_ID_EVIDENCIA;
    DELETE FROM bbf_capacitacion_evidencias
    WHERE ID_CAPACITACION_SESION=P_ID_SESION AND ID_CAPACITACION_EVIDENCIA=P_ID_EVIDENCIA;
END$$
...
-- TOTAL_PROCEDIMIENTOS_CAPACITACIONES = 26
