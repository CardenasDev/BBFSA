...
@Component({
  ...
  template: `
    ...
    @if (createOpen()) { <section class="panel modal"><div class="modal-content"><header><h2>{{ editMode() ? 'Editar artículo' : 'Agregar artículo' }}</h2><button class="icon-btn" type="button" aria-label="Cerrar" (click)="closeCreate()">×</button></header>
      <p class="muted">El código se asigna automáticamente al crear el artículo.</p>
      <form (ngSubmit)="save()">
...
export class ParametersUniformItemsComponent implements OnInit {
  ...
  openCreate(){ this.formError.set(''); this.form={nombre:'',id_tipo_dotacion:this.families()[0]?.id_tipo_dotacion ?? null,requiere_talla:false}; this.editingId=null; this.editMode.set(false); this.createOpen.set(true); this.showFormAtTop(); }
  openEdit(it:any){ this.formError.set(''); this.form={nombre:it.nombre,id_tipo_dotacion:it.id_tipo_dotacion,requiere_talla:!!it.requiere_talla}; this.editingId=it.id_articulo; this.editMode.set(true); this.createOpen.set(true); this.showFormAtTop(); }
  ...
  private showFormAtTop(): void { queueMicrotask(() => window.scrollTo({ top: 0, behavior: 'smooth' })); }
}
