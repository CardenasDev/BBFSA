...
@Component({
  ...
  template: ` <div class="admin-shell" [class.sidebar-open]="menuOpen()">
    <aside class="sidebar">
      <div class="sidebar-brand">
        <span class="brand-mark small">BBF</span>
        <div><strong>Barro Blanco</strong><small>Sistema Administrativo</small></div>
      </div>
      <nav>
      ...
      <div class="sidebar-footer">
        ...
        <small class="app-version sidebar-version" aria-label="Versión de la aplicación">{{
          appVersion
        }}</small>
      </div>
    </aside>
    <div class="page-shell">
      <header class="topbar">
        ...
        <div class="topbar-title">
          <strong>Sistema Administrativo</strong><small>Barro Blanco Farms</small>
        </div>
        <div class="user-menu">
          <div class="avatar">{{ initials() }}</div>
          <div class="user-copy">
            <strong>{{ auth.currentUser()?.nombre_usuario }}</strong
            ><small>{{ auth.currentUser()?.tipo_usuario }}</small>
          </div>
          <button class="btn ghost" (click)="logout()" [disabled]="loggingOut()">Salir</button>
        </div>
      </header>
      <main class="content"><router-outlet /></main>
    </div>
    @if (menuOpen()) {
      <button class="overlay" (click)="menuOpen.set(false)" aria-label="Cerrar menu"></button>
    }
  </div>`,
})
export class AdminLayoutComponent {
  ...
  private readonly items: MenuItem[] = [
    ...
    { label: 'Empleados', icon: 'E', route: '/admin/employees', permissions: ['EMPLEADOS_VER'] },
    {
      label: 'Carga masiva',
      icon: 'CM',
      route: '/admin/bulk-load',
      permissions: ['EMPLEADOS_VER', 'EMPLEADOS_CREAR'],
    },
    {
      label: 'Herramientas',
      icon: 'H',
      ...
      children: [
        {
          ...
          icon: 'C',
          route: '/admin/tools',
          permissions: ['HERRAMIENTAS_LISTAR'],
        },
        {
          label: 'Entregas',
          icon: 'E',
          route: '/admin/tool-deliveries',
          permissions: ['HERRAMIENTAS_LISTAR'],
        },
        {
          label: 'Registrar entrega',
          icon: '+',
          route: '/admin/tool-deliveries/create',
          permissions: ['HERRAMIENTAS_ENTREGAR'],
        },
      ],
    },
    {
      ...
      children: [
        {
          ...
          icon: 'M',
          route: '/admin/dotations/my-sizes',
          permissions: ['DOTACIONES_MIS_TALLAS_VER'],
        },
        {
          label: 'Mis dotaciones',
          icon: 'D',
          route: '/admin/dotations/my-deliveries',
          permissions: ['DOTACIONES_MIS_ENTREGAS_VER'],
        },
        {
          label: 'Mis herramientas',
          icon: 'H',
          route: '/admin/my-tool-deliveries',
          permissions: ['HERRAMIENTAS_MIS_ENTREGAS_VER'],
        },
        {
          label: 'Control de dotaciones',
          icon: 'C',
          route: '/admin/dotations/employees',
          permissions: ['DOTACIONES_ADMIN_VER'],
        },
        {
          label: 'Devoluciones',
          icon: 'R',
          route: '/admin/returns',
          permissions: ['DEVOLUCIONES_VER'],
        },
      ],
    },
    {
      ...
      menuKey: 'trainings',
      permissions: ['CAPACITACIONES_VER', 'CAPACITACIONES_MIS_REGISTROS_VER'],
      children: [
        {
          label: 'Mis capacitaciones',
          icon: 'M',
          route: '/admin/trainings/my-records',
          permissions: ['CAPACITACIONES_MIS_REGISTROS_VER'],
        },
        {
          label: 'Sesiones',
          icon: 'S',
          route: '/admin/trainings/sessions',
          permissions: ['CAPACITACIONES_VER'],
        },
        {
          label: 'Catálogo',
          icon: 'C',
          route: '/admin/trainings/catalog',
          permissions: ['CAPACITACIONES_VER'],
        },
        {
          label: 'Alertas y compromisos',
          icon: 'A',
          route: '/admin/trainings/follow-up',
          permissions: ['CAPACITACIONES_VER', 'CAPACITACIONES_COMPROMISOS'],
        },
      ],
    },
    {
      ...
      children: [
        ...
        },
        {
          label: 'Alertas',
          icon: 'A',
          route: '/admin/contracting/alerts',
          permissions: ['CONTRATACION_ALERTAS_VER'],
        },
      ],
    },
    {
      ...
      icon: 'U',
      route: '/admin/users',
      permissions: ['USUARIOS_LISTAR', 'USUARIOS_VER'],
    },
    {
      label: 'Roles',
      icon: 'R',
      route: '/admin/roles',
      permissions: ['ROLES_LISTAR', 'ROLES_VER'],
    },
  ...
  private updateOpenMenuFromUrl(url: string): void {
    ...
    if (path.startsWith('/admin/trainings')) {
      this.openMenu.set('trainings');
      return;
    }
  ...
  initials(): string {
    return (this.auth.currentUser()?.nombre_usuario ?? 'U')
      .split(/\s+/)
      .slice(0, 2)
      .map((part) => part[0])
      .join('')
